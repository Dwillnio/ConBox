...
void pid_osc(QLineSeries* data_x, QLineSeries* data_u)
{
    ...
    func_dgl function(func_osc_contrl, 2, 1, 0);
    vec x(2);
    x[0] = 1;
    x[1] = 0;
...
int main(int argc, char** argv)
{
    ...
    matrix a(std::vector<rnum>({-1/sqrt(2),-1/sqrt(2),0,1+1/sqrt(2),-1/sqrt(2),0,0,0,1}), 3);
    matrix temp(m);
    matrix b({12,-51,4,6,167,-68,-4,24,-41}, 3);
    std::cout << 2*matrix::unit(4);
    //std::cout << a.det() << "\n";
    //std::cout << a.inverse();
    b = a;
    std::cout << b;
    ...
    func_dgl func(func_osc_contrl, 2, 1, 0);
